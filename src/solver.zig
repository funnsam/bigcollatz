// split off a part of the bigint to solve, and bulk update it when the divided part is about to overflow by keeping track of operations and update by (mul*num + add) >> rsh
...
pub fn solve(_num: ubig) !void {
    ...
    while (!std.mem.eql(ubig.Digit, num.digits.items, &.{1})) {
        const update = solveSmall(num.digits.items[0] & small_chunk_mask);

        try num.mulAddSd(update.mul, update.add);
